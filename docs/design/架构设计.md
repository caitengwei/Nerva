# Nerva 架构设计

更新时间：2026-03-03

本文按当前仓库实现整理，重点回答三个问题：
- Nerva 的核心链路是什么。
- 各层分别负责什么。
- 出问题时应该先看哪里。

## 1. 设计目标（工程视角）

Nerva 的目标很直接：让多模型编排和推理服务能在同一套 Python 框架里高效运行，同时还具备灵活快速的开发效率。

这背后有三条约束：
- 编排要灵活：用户可以用普通函数风格定义 DAG。
- 执行要稳定：模型运行放到独立 Worker 进程，避免互相拖垮。
- 诊断要清晰：指标、日志、错误码需要能对应到具体阶段。

## 2. 总体架构

```mermaid
flowchart TB
    subgraph Client[客户端]
      C[Binary RPC Client]
    end

    subgraph Master[服务主进程]
      APP[ASGI App]
      RPC[RpcHandler]
      PIPE[_PipelineExecutor]
      EXE[Executor]
      WM[WorkerManager]
      WPA[WorkerProxy A]
      WPB[WorkerProxy B]
      OBS[Metrics + Logging]
    end

    subgraph WorkerA[Worker 进程 A]
      LOOPA[_WorkerLoop]
      BEA[Backend]
      MA[Model]
    end

    subgraph WorkerB[Worker 进程 B]
      LOOPB[_WorkerLoop]
      BEB[Backend]
      MB[Model]
    end

    SHM[(Shared Memory Pool)]

    C --> APP --> RPC --> PIPE --> EXE
    EXE --> WPA
    EXE --> WPB
    WM --> WPA
    WM --> WPB

    WPA <-->|ZeroMQ + msgpack| LOOPA
    WPB <-->|ZeroMQ + msgpack| LOOPB

    LOOPA --> BEA --> MA
    LOOPB --> BEB --> MB

    WPA -.大 payload.-> SHM
    WPB -.大 payload.-> SHM

    RPC --> OBS
    EXE --> OBS
    WM --> OBS
```

## 3. 一次请求的主链路

以 `POST /rpc/{pipeline}` 为例：

1. `RpcHandler` 校验 header 和二进制帧格式。
2. 把绝对 deadline 转成相对 TTL，生成请求上下文。
3. `_PipelineExecutor` 创建 `Executor` 执行 DAG。
4. `Executor` 按依赖调度节点，调用对应 `WorkerProxy.infer`。
5. `WorkerProxy` 通过 IPC 把请求发给 Worker。
6. Worker 执行 `Backend.infer`，再返回 ACK。
7. `Executor` 收敛结果，`RpcHandler` 组帧回包。

```mermaid
sequenceDiagram
    participant CL as Client
    participant RH as RpcHandler
    participant EX as Executor
    participant PR as WorkerProxy
    participant WL as WorkerLoop
    participant BE as Backend

    CL->>RH: POST /rpc/{pipeline}
    RH->>RH: parse + validate
    RH->>EX: execute(inputs, deadline_ms)
    EX->>PR: infer(node_inputs)
    PR->>WL: INFER_SUBMIT
    WL->>BE: infer(inputs, context)
    BE-->>WL: result
    WL-->>PR: INFER_ACK
    PR-->>EX: node output
    EX-->>RH: final output
    RH-->>CL: DATA + END
```

## 4. 分层职责

| 层 | 主要文件 | 责任 |
|---|---|---|
| DSL/图层 | `core/model.py`, `core/proxy.py`, `core/graph.py`, `core/primitives.py` | 从函数定义构图，表达依赖和控制流 |
| 执行层 | `engine/executor.py`, `engine/batcher.py` | DAG 调度、批处理、请求级上下文传递 |
| 进程/IPC 层 | `worker/manager.py`, `worker/proxy.py`, `worker/process.py`, `worker/ipc.py` | worker 生命周期与跨进程通信 |
| 服务层 | `server/protocol.py`, `server/rpc.py`, `server/serve.py`, `server/app.py` | 对外协议、请求治理、服务装配 |
| 后端层 | `backends/base.py`, `backends/pytorch.py`, `backends/vllm.py` | 模型后端抽象与具体实现 |
| 可观测层 | `observability/metrics.py`, `observability/logging.py` | 指标与结构化日志 |

## 5. 并发与隔离模型

这一节单独展开并发设计，因为它直接决定延迟、吞吐和故障形态。

### 5.1 请求级并发（DAG 调度）

`Executor` 维护 in-degree 表和完成队列。运行时逻辑是：
- 入度为 0 的节点先启动。
- 节点完成后，后继节点入度减 1。
- 入度降到 0 就立即调度。

这让 DAG 能按依赖并行推进，而不是固定串行。

### 5.2 节点级并发（控制流节点）

- `parallel` 节点：分支子图并发执行，结果按分支索引汇总。
- `cond` 节点：先算 predicate，再执行命中分支，未命中分支不执行。

也就是说，并发不只发生在“多个请求之间”，同一请求内部的图分支也会并行。

### 5.3 进程级并发（Worker 侧）

每个 Worker 是独立进程。`_WorkerLoop` 收到 `INFER_SUBMIT` 后会创建任务执行推理，并处理：
- `CANCEL`：标记上下文并取消任务。
- deadline：超时返回 `DEADLINE_EXCEEDED`。
- SHM 分配交互：大输出时向主进程申请槽位。

这个模型的好处是故障边界清晰：单个模型进程异常，不会直接污染其他模型执行状态。

### 5.4 资源并发控制

并发不是无限放开，系统里有几道“闸门”：
- `DynamicBatcher.queue_capacity`：限制排队深度。
- `queue_timeout_ms` + deadline：限制排队等待时间。
- SHM size class + slot：限制大 payload 并发占用。

触发上限时会走显式失败（例如 `RESOURCE_EXHAUSTED`），而不是隐式堆积到超时雪崩。

### 5.5 当前部署模型与分布式扩展位点

当前实现口径是单机多进程（一个服务主进程 + 多个 Worker 进程）。
如果后续要扩到分布式，建议沿现有边界外扩，而不是推倒重写：
- 把 `RpcHandler` 前面加网关层（接入与限流）。
- 把 `WorkerManager` 的“本机进程管理”抽象成“远端执行单元注册/探活”。
- 保留 `Executor` 的 DAG 语义，替换节点执行位置解析逻辑（本机 -> 远端）。

```mermaid
flowchart LR
    GW[Gateway] --> ORCH[Orchestrator]
    ORCH --> EX1[Executor on Node A]
    ORCH --> EX2[Executor on Node B]
    EX1 --> WA[Workers A]
    EX2 --> WB[Workers B]
```

上图是扩展方向示意，不代表当前仓库已经实现分布式调度。

## 6. IPC 设计

- 控制通道：ZeroMQ PAIR + msgpack，传控制消息和 descriptor。
- 数据通道：小 payload inline，大 payload 走 SHM。

当前 inline 阈值：`IPC_CONTROL_INLINE_MAX_BYTES = 8 * 1024`。

```mermaid
flowchart LR
    P[payload] --> Q{<= 8KB}
    Q -- 是 --> I[inline_data]
    Q -- 否 --> S[alloc SHM slot + write]
    I --> M[INFER_SUBMIT]
    S --> M
```

## 7. 错误处理策略

### 7.1 RPC 层错误码

- `INVALID_ARGUMENT (3)`：协议或参数问题。
- `DEADLINE_EXCEEDED (4)`：请求超时或已过期。
- `RESOURCE_EXHAUSTED (8)`：资源不足（排队/容量压力）。
- `INTERNAL (13)`：未分类内部错误。

### 7.2 执行层策略

- 节点异常直接终止本次 DAG。
- 取消剩余任务，避免无效计算继续占资源。

### 7.3 Worker 生命周期

- 启动失败会回滚清理。
- 支持重启（有限次数）。
- 关停阶段走 best-effort，最终保证进程回收。

## 8. 可观测性

关键指标（部分）：
- `nerva_request_total`
- `nerva_request_duration_seconds`
- `nerva_request_in_flight`
- `nerva_batch_size`
- `nerva_batch_wait_seconds`
- `nerva_queue_depth`
- `nerva_worker_status`

日志通过 `request_id` 与 `pipeline` 绑定上下文。排障时建议用 `request_id` 把入口日志、执行器日志、worker 日志串起来看。

## 9. 当前边界

当前主路径是 unary Binary RPC；`x-nerva-stream` 仅支持 `0`。框架重点覆盖单机进程模型，复杂控制平面（例如模型仓库治理、租户策略）不在当前实现范围内。

## 10. 回归风险提醒

已知高风险点（文档口径）：`R-PH2-PROXY-CAPTURE`。

建议持续保留这两类回归：
- `out = a(x); cond(out["flag"], lambda: b(out), lambda: c(out))`
- `out = a(x); parallel(lambda: b(out), lambda: c(out))`
