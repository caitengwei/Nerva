...
# 端到端
uv run pytest tests/test_server_rpc_e2e.py tests/test_serve.py tests/test_observability_vllm_e2e.py -v
```
...
| Executor 语义 | `tests/test_executor.py`, `tests/test_dag_executor_e2e.py` | fail-fast、cond/parallel 行为 |
| Worker IPC | `tests/test_worker_proxy.py`, `tests/test_worker_process.py`, `tests/test_worker_ipc_e2e.py` | descriptor、deadline、cancel、SHM |
| 服务协议 | `tests/test_protocol.py`, `tests/test_rpc.py` | frame/header 校验、错误码映射 |
| 全链路 | `tests/test_server_rpc_e2e.py`, `tests/test_serve.py` | Binary RPC + real worker |
...
```bash
uv run pytest tests/test_primitives.py tests/test_executor.py tests/test_dag_executor_e2e.py -v
```

### 改 `worker/*`

```bash
uv run pytest tests/test_worker_proxy.py tests/test_worker_process.py tests/test_worker_manager.py tests/test_worker_ipc_e2e.py -v
```

### 改 `server/rpc.py` 或 `server/protocol.py`

```bash
uv run pytest tests/test_protocol.py tests/test_rpc.py tests/test_server_rpc_e2e.py -v
```
